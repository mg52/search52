...
func newAPIMux(ht *handler.HTTP) *http.ServeMux {
	...
	mux.HandleFunc("/persist-category-embed", requireAdminKey(ht.PersistCategoryEmbed))
...
func newAdminMux(ht *handler.HTTP) *http.ServeMux {
	...
	mux.HandleFunc("/api/persist-category-embed", requireAdminKey(ht.PersistCategoryEmbed))
